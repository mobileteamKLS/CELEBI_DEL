...
function GetShipmentForwardingDetails() {
...
function SaveForwardDetails() {
    ...
    if ($('#ddlExamLoc').val() == "0") {
        errmsg = "Please select exam location.</br>";
        $.alert(errmsg);
        return;
    }
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: "POST",
            url: CMSserviceURL + "SaveImpForwardingForExaminationDetails_PDA",
            data: JSON.stringify({
                'pi_intHAWBId': HAWBid, 'pi_strBCNo': $('#txtBCNo').val(), 'pi_intIGMno': IGMNo,
                'pi_strCurrentLocation': currentLoc, 'pi_intTotPieces': TotalPieces, 'pi_strNewLocation': $('#ddlExamLoc').val(),
                'pi_intExamPieces': $('#txtExamPkgs').val(), 'pi_strLoaderId': '0', 'pi_struserId': window.localStorage.getItem("UserID"),
            }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            beforeSend: function doStuff() {
