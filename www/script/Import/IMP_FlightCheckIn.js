...
function saveFlightCheckInDetals() {
    ...
    if ($("#txtScanMAWB").val() == '') {
        ...
        if (HouseNo == '') {
            ...
            // change with krunal 20072022
            inputxml = "<Root><GroupId>" + $("#txtGroupCode").val() + "</GroupId><FlightSeqNo>" + saveFltSeqNo + "</FlightSeqNo><AWBId>" + saveIMPSHIPROWID + "</AWBId><HAWBId>" + HWABIDSEND.split('~')[0] + '~' + IMPSHIPROWID + "</HAWBId><NPR>" + $("#txArrivedPakg").val() + "</NPR><UserId>" + Userid + "</UserId><AirportCity>" + SHED_AIRPORT_CITY + "</AirportCity><NOG>" + $("#txtNOG").val() + "</NOG><CommSrNo>" + $("#ddlComCode").val() + "</CommSrNo></Root>";
            SaveImportMaifestDetailsV2(inputxml);
        }
    }
}
